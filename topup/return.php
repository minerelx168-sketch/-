    ...
    try {
        $stmt = db()->prepare(
            'SELECT public_id, amount, currency, status, paid_at, credited_at
             FROM topup_orders
             WHERE provider_charge_id = ? AND user_id = ?
             LIMIT 1'
        );
...
?>
        ...
        <div class="container container--narrow">
            ...
            <?php if (!$order): ?>
                <div class="topup-status topup-status--pending">
                    ...
                    <p>We're confirming your top-up with Stripe. This usually takes a few seconds.</p>
            ...
            <?php else: ?>
                ...
                <div class="topup-status topup-status--<?= htmlspecialchars($statusClass, ENT_QUOTES, 'UTF-8') ?>"
                    ...
                    <?php elseif ($status === 'FAILED'): ?>
                        ...
                        <p>Stripe reported a payment failure. No credit was added to your wallet.</p>
    ...
    (function () {
        ...
        var tries = 0;
        var maxTries = 30; // ~ 90 seconds at 3s interval
